...
A Figura \ref{fig:vtol_tipos} apresenta três formatos comuns de VTOL, o primeiro na imagem (a) é o \textit{tailsitter}, o qual muda a orientação da aeronave para mudar de modo, enquanto a imagem (b) é o \textit{tiltrotor}, o qual decola na vertical e dobra a orientação apenas do motor para trocar para asa fixa. Esta dissertação aborda o formato da imagem (c), chamado de configuração híbrida, a qual mantém a orientação do motor e simplifica o processo de transição. \textcite{Graesdal2021} estuda uma aeronave dessa configuração, mas concentra a identificação e a validação com dados de voo nas dinâmicas longitudinal e laterodirecional do modo asa fixa, tratando o modo multirrotor apenas de forma teórica, sem estimação de parâmetros nem validação experimental. O presente trabalho ocupa justamente esse espaço complementar, pois identifica e valida com dados de voos reais o modelo do modo multirrotor, responsável pela decolagem e pelo pouso verticais.
...
\begin{enumerate}
    ...
    Desenvolver o modelo dinâmico do drone híbrido operando como quadrirrotor, tratando-o como um corpo rígido e isolando os efeitos aerodinâmicos do modo asa fixa. Esta etapa abrange as equações de movimento rotacional e translacional e o modelo do motor. A estrutura matemática resultante define os parâmetros que a identificação estima a partir dos dados de voo, conforme a formulação de caixa cinza adotada. É esse único modelo, com estrutura teórica e parâmetros identificados experimentalmente, que serve de base para o projeto de controle.
    ...
    Projetar a malha de controle em arquitetura em cascata, com controladores PID para atitude, altitude e velocidade horizontal, sobre o modelo identificado e linearizado em torno do voo pairado. A avaliação do desempenho é realizada exclusivamente por simulação, sem ensaios em voo do controlador. Para isso, a malha fechada é simulada nas plantas linear e não linear, e o contraste entre as respostas delimita a faixa de manobras em que a aproximação linear permanece representativa.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Capítulo 2:} desenvolve a modelagem matemática da aeronave em modo quadrirrotor, abordando os sistemas e as transformações de coordenadas, as equações de movimento (modelos rotacional e translacional e a cinemática de Euler) e o modelo do motor.

    \item \textbf{Capítulo 3:} descreve a plataforma física e a instrumentação do VTOL, abordando a aeronave e sua geometria, o sistema de propulsão (motor e ESC) e a aviônica embarcada (controladora de voo Pixhawk, sensores inerciais e receptor GPS).

    \item \textbf{Capítulo 4:} apresenta a estratégia para a identificação do drone híbrido em modo quadrirrotor. Discute o tratamento de sistemas instáveis e detalha as etapas da metodologia M4V adotada: a definição das manobras, a aquisição dos dados de saída, os métodos de estimação, com ênfase no OEM, o modelo não linear e os critérios de validação no domínio do tempo.

    \item \textbf{Capítulo 5:} avalia os resultados obtidos. Começa pela verificação da coerência dos dados de entrada, seguida da identificação do modelo não linear via OEM e da sua validação. Com o modelo identificado, apresenta o projeto do controlador em cascata (arquitetura, controladores PID e ganhos), a simulação da malha fechada no modelo linear e a comparação do controle aplicado às plantas linear e não linear.
