...
Apesar das vantagens físicas acumuladas dos dois modos, a complexidade dos modelos de representação desse sistema torna-se mais elevada, o que dificulta o projeto de controle, como apresenta \textcite{Graesdal2021}. Essa dificuldade decorre da união dos efeitos do modo multirrotor com a aerodinâmica do modo asa fixa, o que {\color{blue}dificulta} a separação da contribuição de cada um na avaliação das medidas dos sensores. O problema é mais evidente na {\color{blue}transição, fase em que as duas dinâmicas atuam de forma simultânea com magnitudes comparáveis, antes de a sustentação da asa se estabelecer e de os motores verticais serem desligados. Para evitar essa sobreposição, a avaliação isolada de cada contribuição exige ensaios nos extremos do envelope, o voo pairado com velocidades de translação próximas de zero, em que a aerodinâmica da estrutura é pouco solicitada, e o voo em modo asa fixa com os motores verticais desligados, em que a contribuição dos rotores é nula. Este trabalho concentra-se no extremo do voo pairado, como parte de uma pesquisa mais ampla sobre a plataforma}.
...
{\color{blue}Aplicar identificação de sistemas a uma aeronave sustentada por rotores em baixa velocidade, contudo, traz dificuldades reconhecidas na literatura. \textcite{Jategaonkar2015} observa que essas aeronaves são mais difíceis de identificar que as de asa fixa, pelo forte acoplamento aerodinâmico entre os eixos e pela baixa qualidade das medidas de escoamento em baixa velocidade e em voo pairado. A razão é que os sensores de fluxo de ar dependem da velocidade da aeronave para funcionar, e no voo pairado essa velocidade é praticamente nula. Por isso, no lugar de uma medida direta do escoamento, este trabalho utiliza a velocidade estimada pelo computador de bordo. Quanto ao método, a identificação no domínio do tempo tem aplicação consolidada em aeronaves de asa rotativa, e \textcite{Tischler2012} reconhece o centro aeroespacial alemão como referência mundial na identificação de helicópteros por máxima verossimilhança.} Neste contexto, é necessária a utilização de técnicas específicas para a identificação de plantas instáveis, como as descritas por \textcite{Jategaonkar2015}. Convém ainda adotar uma metodologia que organize o processo completo, papel cumprido pela M4V, que vai do projeto das manobras de excitação à validação do modelo. Suas etapas intermediárias incluem o tratamento das medidas obtidas e a estimação de parâmetros com dados de voo, por métodos como o \gls{EEM} e o \gls{OEM}, avaliados segundo critérios quantitativos estabelecidos.
