Este capítulo descreve a plataforma física empregada no trabalho e a instrumentação responsável pela aquisição dos dados de voo, abordando a aeronave e sua geometria, o sistema de propulsão e a aviônica embarcada. {\color{blue}Em seguida, apresenta a metodologia de identificação adotada, formulando o problema nas classes de caixa branca, cinza e preta, tratando da identificação de sistemas instáveis em malha fechada e revendo os métodos de estimação empregados na prática aeronáutica. A execução dessa metodologia sobre o drone híbrido é detalhada no Capítulo~\ref{chap:identificacao}.}
