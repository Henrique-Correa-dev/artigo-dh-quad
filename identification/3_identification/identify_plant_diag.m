% identify_plant_diag.m — CÓPIA DE DIAGNÓSTICO de identify_plant.m  (GERADO por make_diag.py)
% =========================================================================
...
% (linha original:) t_val   % (antes 605–625) — janela de validação oficial
...
function [aX, aY, aZ] = accel_eval(p, q, r, u, v, w, pwm, k_T, func_T_ref, m, r_imu, dt)
...
%  com IMU offset. Saída: accX/accY/accZ.
    ...
    [aX, aY, aZ] = accelerometer_model(p, q, r, u, v, w, T./m, ...
        gradient(p, dt), gradient(q, dt), gradient(r, dt), r_imu);
end
    ...
    for w = 1:length(win_starts)
        ...
        for k = i_s:i_e-1
            ...
            for si = 1:n_sub
                ...
                % k1
                pd1 = G1*ps*qs - G2*qs*rs + G3*Mxa + G4*Mza - Dp*ps;
                qd1 = G5*ps*rs - G6*(ps^2 - rs^2) + invJy*Mya - Dq*qs;
                rd1 = G7*ps*qs - G1*qs*rs + G4*Mxa + G8*Mza - Dr*rs;
                % k2
                p2 = ps+h2*pd1; q2 = qs+h2*qd1; r2 = rs+h2*rd1;
                pd2 = G1*p2*q2 - G2*q2*r2 + G3*Mxb + G4*Mzb - Dp*p2;
                qd2 = G5*p2*r2 - G6*(p2^2 - r2^2) + invJy*Myb - Dq*q2;
                rd2 = G7*p2*q2 - G1*q2*r2 + G4*Mxb + G8*Mzb - Dr*r2;
                % k3
                p3 = ps+h2*pd2; q3 = qs+h2*qd2; r3 = rs+h2*rd2;
                pd3 = G1*p3*q3 - G2*q3*r3 + G3*Mxb + G4*Mzb - Dp*p3;
                qd3 = G5*p3*r3 - G6*(p3^2 - r3^2) + invJy*Myb - Dq*q3;
                rd3 = G7*p3*q3 - G1*q3*r3 + G4*Mxb + G8*Mzb - Dr*r3;
                % k4
                p4 = ps+dt_sub*pd3; q4 = qs+dt_sub*qd3; r4 = rs+dt_sub*rd3;
                pd4 = G1*p4*q4 - G2*q4*r4 + G3*Mxc + G4*Mzc - Dp*p4;
                qd4 = G5*p4*r4 - G6*(p4^2 - r4^2) + invJy*Myc - Dq*q4;
                rd4 = G7*p4*q4 - G1*q4*r4 + G4*Mxc + G8*Mzc - Dr*r4;
                % update
...
% Nenhuma interpolação entre etapas, nenhuma reamostragem.
    ...
    try
        ...
        [accX, accY, accZ] = accel_eval(y_out(:,1), y_out(:,2), y_out(:,3), ...
            ...
            constants_sim.m, parameters().imu_offset, time(2)-time(1));
...
% Retorna struct com mesmos campos que simulate_full (compatível com plots).
    ...
    for d = 1:2
        ...
        [accX, accY, accZ] = accel_eval(pqr(:,1), pqr(:,2), pqr(:,3), ...
            u_int, v_int, w_int, pwm, k_T, func_T_ref, m, r_imu_sc, dt);
...
% Diferente do semi-acoplado: pqr aqui é INTEGRADO, não passthrough.
    ...
    for d = 1:2
        ...
        [accX, accY, accZ] = accel_eval(p_sim, q_sim, r_sim, ...
            u_int, v_int, w_int, pwm, k_T, func_T_ref, m, r_imu_hyb, dt);
...
function plot_torques(P, time_vl, pwm_vl, func_T_ref, func_Q_ref, pqr_vl, t_val)
    ...
    saveas(fig3, fullfile(getappdata(0,'diag_img_dir'), '05_torques.png'));
...
function plot_forces(P, time_vl, pwm_vl, att_vl, acc_vl, func_T_ref, constants_sim, t_val, res)
    ...
    saveas(fig4, fullfile(getappdata(0,'diag_img_dir'), '06_forcas.png'));
    ...
    saveas(fig5, fullfile(getappdata(0,'diag_img_dir'), '04_pwm.png'));
