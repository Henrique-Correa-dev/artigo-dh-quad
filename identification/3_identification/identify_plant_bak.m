...
% Atraso modelo×medido (~1 amostra/100 ms, ver lag_diagnostic). Mesmo mecanismo
% do identify_v2: LAG_TAU>0 = lag 1ª ordem (arredonda+atrasa, p/ testar lag de
% motor); senão DELAY_PWM = atraso puro (shift). 0/0 = desliga.
% LAG_TAU=0.15 TESTADO: embranquece p (mata a oscilação 0.65 Hz) MAS piora a
% validação (p TIC 0.31→0.35, R² 0.63→0.51) e puxa Jx,Jy p/ baixo (triângulo
% volta a violar -0.6%). O lag de 1ª ordem ATENUA amplitude além de defasar →
% troca erro de fase por erro de amplitude. O atraso PURO (1 amostra, all-pass)
% é o melhor ponto de operação. Lag confirmou a FÍSICA (existe lag motor/aero),
% mas o ganho real agora é ESTRUTURAL (re-medir Q, aero residual), não no filtro.
DELAY_PWM = 1;   % amostras (atraso puro) — melhor ponto de operação
LAG_TAU   = 0;   % s — lag 1ª ordem; ver nota acima (super-corrige)
if LAG_TAU > 0
    a_lag = exp(-dt/LAG_TAU);  lp = @(x) filter(1-a_lag,[1 -a_lag],x);
    pwm1_interp=lp(pwm1_interp); pwm2_interp=lp(pwm2_interp);
    pwm3_interp=lp(pwm3_interp); pwm4_interp=lp(pwm4_interp);
    fprintf('  >>> PWM com LAG 1ª ordem τ=%.3f s\n', LAG_TAU);
elseif DELAY_PWM > 0
    dl = @(x) [repmat(x(1),1,DELAY_PWM), x(1:end-DELAY_PWM)];  % x é linha (1xN)
    pwm1_interp=dl(pwm1_interp); pwm2_interp=dl(pwm2_interp);
    pwm3_interp=dl(pwm3_interp); pwm4_interp=dl(pwm4_interp);
    fprintf('  >>> PWM com atraso PURO de %d amostra(s)\n', DELAY_PWM);
end
...
saveas(figW, fullfile(paths.images,'identify_plant_whiteness.png'));
fprintf('  (≥90%% dentro da banda ≈ branco. Figura: %s)\n', fullfile(paths.images,'identify_plant_whiteness.png'));
